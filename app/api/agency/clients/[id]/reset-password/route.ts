...
export async function POST(
  ...
  try {
    ...
    // Build baseUrl from request headers (preserves agency subdomain branding)
    const baseUrl = request.headers.get('origin') || process.env.NEXT_PUBLIC_APP_URL || process.env.NEXTAUTH_URL || 'http://localhost:3000';
    const resetUrl = `${baseUrl}/reset-password?token=${token}`;
    
    // Cross-tenant safety: resolve recipient strictly from target user record (not request body)
    // This matches v6.3 email security rules
    const recipient = client.email; // From database, not request
    
    try {
      const emailTemplate = passwordResetEmail(client.name ?? 'there', resetUrl);
      await sendEmail({
        to: recipient,
      ...
      console.log(JSON.stringify({
        event: 'AGENCY_CLIENT_PASSWORD_RESET_EMAIL_SENT',
        agencyId: agency.id,
        clientId: client.id,
        recipient,
