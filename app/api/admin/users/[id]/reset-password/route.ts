...
export async function POST(req: Request, { params }: { params: { id: string }}) {
  try {
    ...
    // Build baseUrl from request headers (host-aware for subdomains)
    const baseUrl = req.headers.get('origin') || process.env.NEXT_PUBLIC_APP_URL || process.env.NEXTAUTH_URL || 'http://localhost:3000';
